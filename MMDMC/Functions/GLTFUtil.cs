...
namespace MMDMC.Functions
{
    public static class GLTFUtil
    {
        ...
        public static void SaveAsGLTF2(PMXFormat pmx, VMDFormat vmd, ExportOptions options, string path)
        {
            ...
            var vertMorphs = pmx.Morphs.Where(u => u.MorphVertice != null).ToArray();

            var vertMorphsIndices = new List<HashSet<int>>();
            foreach (var p in vertMorphs)
            {
                vertMorphsIndices.Add(p.MorphVertice.Select(u => u.VertexIndex).ToHashSet());
            }
            ...
            if (true)
            {
                ...
                else
                {
                    for (int i = 0; i < vertex.Length; i++)
                    {
                        writer.Write((vertex[i].boneId0 >= 0) ? (byte)vertex[i].boneId0 : (byte)0);
                        writer.Write((vertex[i].boneId1 >= 0) ? (byte)vertex[i].boneId1 : (byte)0);
                        writer.Write((vertex[i].boneId2 >= 0) ? (byte)vertex[i].boneId2 : (byte)0);
                        writer.Write((vertex[i].boneId3 >= 0) ? (byte)vertex[i].boneId3 : (byte)0);
                    }
                    glTFWriterContext.MarkBuf("_joints", new GLTFBufferView { }, new GLTFAccessor { componentType = 5121, count = pmx.Vertices.Length, type = "VEC4" });
                }
            ...
            {
                var morph = vertMorphs[i].MorphVertice;
        ...
        static void WriteVectors(Vector3[] vec3s, BinaryWriter writer, out float[] min, out float[] max)
        {
            min = new float[3];
            max = new float[3];
        ...
        static void WriteVectors(Vector2[] vec2s, BinaryWriter writer, out float[] min, out float[] max)
        {
            min = new float[2];
            max = new float[2];
        ...
        static void WriteFloats(float[] vec2s, BinaryWriter writer, out float[] min, out float[] max)
        {
            min = new float[1];
            max = new float[1];
