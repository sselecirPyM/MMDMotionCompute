...
namespace MMDMC.MMD
{
    ...
    public class VMDFormat
    {
        ...
        public static VMDFormat Load(byte[] bytes)
        {
        ...
        void ReadBoneFrames(BinaryReader reader, Encoding jpEncoding)
        {
            ...
            for (int i = 0; i < numBone; i++)
            {
                ...
                keyFrame.translation = ReadVector3ZInv(reader);
                keyFrame.rotation = ReadQuaternionXYInv(reader);
        ...
        void ReadCameraFrames(BinaryReader reader)
        {
            ...
            for (int i = 0; i < numCam; i++)
            {
                ...
                keyFrame.position = ReadVector3ZInv(reader);
                keyFrame.rotation = ReadVector3XYInv(reader);
        ...
        void ReadLightFrames(BinaryReader reader)
        {
            ...
            for (int i = 0; i < numLight; i++)
            {
                ...
                lightKeyFrame.Position = ReadVector3ZInv(reader);
        ...
        public void SaveToFile(BinaryWriter writer)
        {
            ...
            foreach (var NKPair in BoneKeyFrameSet)
            {
                ...
                foreach (var keyFrame in NKPair.Value)
                {
                    ...
                    WriteVector3ZInv(writer, keyFrame.translation);
                    WriteQuaternionXYInv(writer, keyFrame.rotation);
            ...
            foreach (var keyframe in CameraKeyFrames)
            {
                ...
                WriteVector3ZInv(writer, keyframe.position);
                WriteVector3ZInv(writer, keyframe.rotation);
            ...
            {
                writer.Write(keyframe.Frame);
                WriteVector3ZInv(writer, keyframe.Color);
                WriteVector3ZInv(writer, keyframe.Position);
            }
        }
        ...
        private void WriteVector3ZInv(BinaryWriter writer, Vector3 vector3)
        {
            writer.Write(vector3.X);
            writer.Write(vector3.Y);
            writer.Write(-vector3.Z);
        }
        private void WriteQuaternionXYInv(BinaryWriter writer, Quaternion quaternion)
        {
