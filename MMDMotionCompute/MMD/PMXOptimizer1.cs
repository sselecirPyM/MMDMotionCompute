...
namespace MMDMotionCompute.MMD
{
    public class PMXOptimizer1
    {
        ...
        /// <returns></returns>
        public PMXFormat OptimizePMX(PMXFormat pmx)
        {
            ...
            foreach (var pair in newVertMorph)
            {
                ...
                pmxOutput.Morphs[pair.Key].MorphVertexs = pair.Value.ToArray();
            }
            //for (int i = 0; i < pmxOutput.Morphs.Count; i++)
            //{
            //    var morph = pmxOutput.Morphs[i];
        ...
        {
            for (int i = 0; i < vertMorph.MorphVertexs.Length; i++)
            {
                int c = vertMorph.MorphVertexs[i].VertexIndex;
                if (!refMap.TryGetValue(c, out var d))
                {
                    d = new List<MorphInvertRef>();
                    refMap[c] = d;
                }
                d.Add(new MorphInvertRef { indexRef = index, vertRef = vertMorph.MorphVertexs[i] });
            }
        }
